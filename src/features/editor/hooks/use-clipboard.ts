...
export const useClipboard = ({ canvas }: UseClipboardProps) => {
    const clipboard = useRef<fabric.Object | fabric.ActiveSelection>(null);
    ...
    const paste = useCallback(() => {
        ...
        clipboard.current.clone(
            (clonedObj: fabric.Object | fabric.ActiveSelection) => {
                ...
                clipboard.current &&
                    (clipboard.current.left =
                        (clipboard.current.left ?? 0) + 10);
